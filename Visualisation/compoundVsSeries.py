...
mainDimensions = ['Time [s]','Average acceptability function value']
adjustToSize = False
fieldNames = ['Moves performed', 'Time [s]', 'Average acceptability function value', 'Average loss', 'Acceptable', 'Action']

seriesFolders = ['solutions/views_starts_ends_trim_heuristic_6loops', 'solutions/compund_6loops']
seriesNames = ['Initial + Views Heur + Starts Heur + Ends Heur + Trim Heur', 'Initial + Compound Heuristic']
...
plt.title(f"{mainDimensions[1]} over time, from {count} runs")
plt.legend()
plt.show()